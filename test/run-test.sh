...
function run_byond_tests {
    ...
    run_test_ci "check globals build" "python tools/GenerateGlobalVarAccess/gen_globals.py InterHippie.dme code/_helpers/global_access.dm"
    run_test "check globals unchanged" "md5sum -c - <<< 'af208a182ec750fa53ed5ccccd5505ff *code/_helpers/global_access.dm'"
    run_test "build map unit tests" "scripts/dm.sh -DUNIT_TEST -M$MAP_PATH InterHippie.dme"
    run_test "check no warnings in build" "grep ', 0 warnings' build_log.txt"
    run_test "run unit tests" "DreamDaemon InterHippie.dmb -invisible -trusted -core 2>&1 | tee log.txt"
