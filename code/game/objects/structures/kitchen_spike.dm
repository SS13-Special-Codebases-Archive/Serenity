...
/obj/structure/kitchenspike/attackby(obj/item/I, mob/living/carbon/human/user)
	if (buckled_mob)
		if (is_sharp(I))
			...
			to_chat(user, "<span class='danger'>You need something sharp to do that!</span>")
		return
	var/obj/item/grab/G = I

	if (!istype(G) || !G.affecting || !can_spike(G.affecting))
		to_chat(user, "<span class='danger'>You can't put that on [src]!</span>")
		return
	if (!G.force_danger())
		to_chat(user, "<span class='danger'>You need better grip to do it!</span>")
		return
...
/obj/structure/kitchenspike/proc/spike(mob/user, mob/living/victim)
	user.visible_message("<span class='danger'>[user] starts putting [victim] onto \the [src]...</span>",
		"<span class='danger'>[user] starts putting [victim] onto \the [src]...</span>",
	...
	victim.visible_message("<span class='danger'>[user] slams [victim] onto \the [src]!</span>",
		"<span class='danger'>[user] slams [victim] onto \the [src]!</span>",
		"<span class='italics'>You hear a squishy wet noise.</span>")
...
/obj/structure/kitchenspike/user_unbuckle_mob(mob/living/user)
	...
	if(M != user)
		M.visible_message(\
			"<span class='warning'>[user] tries to pull [M] free of \the [src]!</span>",\
			"<span class='warning'>[user] is trying to pull you off \the [src], opening up fresh wounds!</span>",\
			"<span class='italics'>You hear a squishy wet noise.</span>")
		unbuckling = TRUE
		if(!do_after(user, delay = 150, target = src))
			if(M && M == buckled_mob)
				M.visible_message(\
				"<span class='warning'>[user] fails to free [M]!</span>",\
				"<span class='warning'>[user] fails to pull you off of \the [src].</span>")
			unbuckling = FALSE
			return
	else
		M.visible_message(\
		"<span class='warning'>[M] struggles to break free from \the [src]!</span>",\
		"<span class='warning'>You struggle to break free from \the [src], exacerbating your wounds!</span>",\
	...
	if(!M || M != buckled_mob)
		return
	M.adjustBruteLoss(30)
	src.visible_message("<span class='danger'>[M] falls free of \the [src]!</span>")
	M.hanging = FALSE
	unbuckle_mob()
...
/obj/structure/kitchenspike/proc/slice(obj/item/I, mob/living/user)
	...
	if (!istype(H))
		to_chat(user, "<span class='notice'>You start to butcher [buckled_mob] with your [I]...</span>")
		...
		to_chat(user, "<span class='notice'>You've finished butchering [buckled_mob].</span>")
	...
	if (BP_IS_ROBOTIC(organ))
		to_chat(user, "<span class='notice'>Can't extract meat from robotic limbs!</span>")
		return
	if (!organ || organ.is_stump())
		to_chat(user, "<span class='warning'>Nuh-uh. Can't extract meat from there!</span>")
		return
	...
	if (istype(C) && C.butchering_capacity <= 0) //workaround for chests not getting stumped/whatever
		to_chat(user, "<span class='warning'>Can't extract any more meat from there!</span>")
		return
	...
	user.visible_message("<span class='notice'>[user] tries to butcher [H]'s [butchered_organ_name]!</span>",\
		"<span class=<'notice'>You try to butcher [H]'s [butchered_organ_name]...</span>",\
		"<span class=<'italics'>You hear a wet squishing noise.</span>")
	...
	if (do_after(user, delay = 20, target = H))
		...
		user.visible_message("<span class='warning'>[user]'ve successfully butchered [H]'s [butchered_organ_name]!</span>",\
			"<span class='notice'>You've successfully butchered [H]'s [butchered_organ_name]...</span>",\
			"<span class='italics'>You hear a squishy wet noise.</span>")
		if (H.can_feel_pain())
			H.emote("scream")
