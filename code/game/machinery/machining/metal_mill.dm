...
/obj/machinery/metal_mill/interact(mob/user as mob)
	...
	if(!disabled)
		...
		if (!milling)
			if (user.skillcheck(user.skills["crafting"], 65, "You try to mill the object but it uselessly falls apart.  You don't think this item was meant to be milled..", "cooking"))
