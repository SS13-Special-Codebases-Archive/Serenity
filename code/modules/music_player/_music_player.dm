...
/obj/item/music_player/proc/set_mode(value)
	...
	playsound(src, mode == (PLAYER_STATE_OFF || PLAYER_STATE_PAUSE) ? GLOB.button_sound[1] : GLOB.button_sound[2], 35)
...
/obj/item/music_player/proc/StartPlaying()
	...
	if(mode == PLAYER_STATE_PAUSE && sound_token)
		...
		sound_token = GLOB.sound_player.PlayLoopingSound(src, sound_id, tape.track.sound, volume = volume, frequency = frequency, range = 13, falloff = 3, prefer_mute = TRUE, streaming = TRUE, ignore_vis = FALSE)
