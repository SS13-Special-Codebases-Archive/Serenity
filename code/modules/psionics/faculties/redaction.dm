...
/decl/psionic_power/redaction/skinsight/invoke(var/mob/living/user, var/mob/living/target)
	...
	. = ..()
	if(.)
		user.visible_message( ("\The [user] rests a hand on \the [target]."))
...
/decl/psionic_power/redaction/cleanse/invoke(var/mob/living/user, var/mob/living/carbon/human/target)
	...
	if(.)
		...
		if(target.radiation)
			to_chat(user,  ("You repair some of the radiation-damaged tissue within \the [target]..."))
		...
		if(target.getCloneLoss())
			to_chat(user,  ("You stitch together some of the mangled DNA within \the [target]..."))
			if(target.getCloneLoss() >= removing)
				target.adjustCloneLoss(-removing)
			else
				target.adjustCloneLoss(-(target.getCloneLoss()))
			return TRUE
		to_chat(user,  ("You can find no genetic damage or radiation to heal within \the [target]."))
		return TRUE
