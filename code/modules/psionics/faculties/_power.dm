...
/decl/psionic_power/proc/invoke(var/mob/living/user, var/atom/target)
	...
	if(user_psi_leech)
		to_chat(user, ("Your power is leeched away by \the [user_psi_leech] as fast as you can focus it..."))
		return FALSE

	if(target.do_psionics_check(cost, user))
		to_chat(user, ("Your power skates across \the [target], but cannot get a grip..."))
		return FALSE
