...
/obj/item/clothing/head/helmet/space/psi_amp/attack_self(var/mob/user)
	...
	if(faculty.id in boosted_faculties)
		...
		if(slots_left <= 0)
			to_chat(user,  ("There are no slots left to install brainboards into."))
			return
	...
	to_chat(user,  ("You [removed ? "remove" : "install"] the [choice] brainboard [removed ? "from" : "in"] \the [src]. There [slots_left!=1 ? "are" : "is"] [slots_left] slot\s left."))
...
/obj/item/clothing/head/helmet/space/psi_amp/proc/deintegrate()
	...
	to_chat(H,  ("You feel a strange tugging sensation as \the [src] begins removing the slave-minds from your brain..."))
	...
	to_chat(H, ("\The [src] chimes quietly as it finishes removing the slave-minds from your brain."))
...
/obj/item/clothing/head/helmet/space/psi_amp/proc/integrate()
	...
	if(LAZYLEN(boosted_faculties) < max_boosted_faculties)
		to_chat(usr,  ("You still have [max_boosted_faculties - LAZYLEN(boosted_faculties)] facult[LAZYLEN(boosted_faculties) == 1 ? "y" : "ies"] to select. Use \the [src] in-hand to select them."))
		return

	var/mob/living/carbon/human/H = loc
	if(!istype(H) || H.head != src)
		to_chat(usr,  ("\The [src] must be worn on your head in order to be activated."))
		return

	canremove = FALSE
	operating = TRUE
	to_chat(H,  ("You feel a series of sharp pinpricks as \the [src] anaesthetises your scalp before drilling down into your brain."))
	playsound(H, 'sound/weapons/circsawhit.ogg', 50, 1, -1)
	...
	to_chat(H,  ("You experience a brief but powerful wave of deja vu as \the [src] finishes modifying your brain."))
	verbs |= /obj/item/clothing/head/helmet/space/psi_amp/proc/deintegrate
	verbs -= /obj/item/clothing/head/helmet/space/psi_amp/proc/integrate
	operating = FALSE
